...
const GOOGLE_APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzj4KJBdbGQqpxB9QvSAm9A1JYBpYAiIauNhLU_axj0gZPGLRJOzfIPZA35JudkYjU7/exec'; 
...
document.getElementById('surveyForm').addEventListener('submit', function(event) {
    ...
    .then(response => {
        // GAS는 성공 시 HTML 응답을 반환할 수 있으므로, 응답 상태만 확인합니다.
        if (response.ok || response.status === 200 || response.type === 'opaque') {
            ...
            document.getElementById('closeModalBtn').onclick = () => {
             ...
             // 서버 오류가 발생한 경우 (Web App이 배포되지 않았거나 URL이 잘못된 경우)
            console.error('GAS Submission Error Status:', response.status);
            alert('데이터 전송에 실패했습니다. Apps Script URL 및 배포 설정을 확인해주세요.');
        }
